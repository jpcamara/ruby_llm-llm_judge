...
[Case-level results](data/luna-direct-vs-openrouter.json) contain IDs, expected labels, choices, probabilities, usage, timings, and retry counts. No input texts or credentials are saved. Run [the analyzer](analyze_luna_direct.py) to recalculate the table. The benchmark used the same 64 AG News and 32 SST-2 cases as the other gem comparisons. Provider load and routing can change, so rerun on representative application decisions before selecting a path.
